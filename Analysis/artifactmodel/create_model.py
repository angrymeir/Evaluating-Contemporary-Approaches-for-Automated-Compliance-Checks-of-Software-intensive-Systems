...
import svg_components as svgc

dimension = namedtuple('dimension', 'width height')
activity_dimension = dimension(200, 100)
artifact_dimension = dimension(35, 50)
activity_spacer = dimension(150, 0)


class Model:
    def __init__(self, graph, filename, act_to_art_map=None):
        self.baseline = 400
        ...
        self.dependencies = []
        self.flows = []
        self.dwg = svgwrite.Drawing(filename, profile='full', debug=False)
    ...
    def model_activity(self, activity):
        self.activities.append(svgc.activity(self.dwg, self.cur_position.values(), size=activity_dimension))
        self.descriptions.append(svgc.description(self.dwg, self.cur_position.values(), size=activity_dimension, name=activity))
        self.update_position(activity_spacer.width, activity_spacer.height)

    def model_artifact(self, artifact):
        self.artifacts.append(svgc.artifact(self.dwg, self.cur_position.values()))

    def to_svg(self):
        markers = svgc.markers(self.dwg)
        for activity in self.activities:
            self.dwg.add(activity)
        for artifact in self.artifacts:
            self.dwg.add(artifact)
        for description in self.descriptions:
            self.dwg.add(description)
        for dependency in self.dependencies:
            self.dwg.add(dependency)
